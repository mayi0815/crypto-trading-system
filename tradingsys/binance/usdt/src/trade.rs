...
use tradingsys::chat::*;
use tradingsys::error::*;
use tradingsys::parser::Parser;
use tradingsys::position::PositionDB;
use tradingsys::rest::Rest;
use tradingsys::tungstenite::Message;
            ...
            Some(tx) => {
                ...
                tokio::spawn(async move {
                        ...
                        .await
                    {
                        Ok(rsp) => {
                            // exchange rej
                            if let Ok(e) = rsp.json::<tradingsys::chat::Error>().await {
                                error!("{:?}", e);
                                let order = Order::new(
                                    id,
                                    symbol,
